...
(async () => {
    try {
        ...
        console.log("✅ Servidor iniciado. 🛡️ Módulo de Suspensión y Edición Activo.");
...
app.post('/public/comentarios', async (req, res) => { try { const { deporte_id, autor, mensaje } = req.body; if (!deporte_id || !mensaje) return res.json({ success: false }); await db.run('INSERT INTO comentarios (deporte_id, autor, mensaje) VALUES (?, ?, ?)', [deporte_id, autor || 'Anónimo', mensaje]); res.json({ success: true }); } catch (e) { res.json({ success: false }); } });

// =======================================================
// LOGIN CON PROTECCIÓN ANTIPAGOS (SUSPENSIÓN DE CLUBES)
// =======================================================
app.post('/login', async (req, res) => {
    const { email, password } = req.body;
    try {
        const user = await db.get(`SELECT u.*, c.nombre as club_nombre, c.estado as club_estado, d.nombre as deporte_nombre, d.imagen as deporte_logo, d.estado as deporte_estado FROM usuarios u LEFT JOIN clubes c ON u.club_id = c.id LEFT JOIN deportes d ON u.deporte_id = d.id WHERE u.email = ?`, [email]);
        if (user) {
            const passwordValida = await bcrypt.compare(password, user.password);
            if (passwordValida) {
                // VERIFICACIÓN DE SUSPENSIONES
                if (user.rol !== 'SYSADMIN' && user.club_estado === 'INACTIVO') {
                    return res.json({ success: false, mensaje: "⛔ ACCESO DENEGADO: El Club se encuentra suspendido del sistema." });
                }
                if ((user.rol === 'SPORTADMIN' || user.rol === 'CAJERO') && user.deporte_estado === 'INACTIVO') {
                    return res.json({ success: false, mensaje: "⚠️ Acceso denegado: Subcomisión suspendida." });
                }
...
// =======================================================
// RUTAS DE CLUBES Y DEPORTES (AHORA SOPORTAN EDICIÓN)
// =======================================================
...
app.put('/clubes/:id', verificarToken, async (req, res) => { try { await db.run('UPDATE clubes SET nombre = ?, logo = ? WHERE id = ?', [req.body.nombre.toUpperCase(), req.body.logo, req.params.id]); res.json({ success: true }); } catch (e) { res.json({ success: false }); } });
app.put('/clubes/:id/estado', verificarToken, async (req, res) => { try { await db.run('UPDATE clubes SET estado = ? WHERE id = ?', [req.body.estado, req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ success: false }); } });
...
app.put('/deportes/:id', verificarToken, async (req, res) => { try { await db.run('UPDATE deportes SET nombre = ?, imagen = ?, club_id = ? WHERE id = ?', [req.body.nombre.toUpperCase(), req.body.imagen, req.body.club_id, req.params.id]); res.json({ success: true }); } catch (e) { res.json({ success: false }); } });
...
app.post('/productos', verificarToken, upload.single('imagen_file'), async (req, res) => { 
    try { 
        ...
        else if (req.body.imagen_url && req.body.imagen_url.trim() !== '') { imagenFinal = req.body.imagen_url.trim(); } 
        else { imagenFinal = `https://ui-avatars.com/api/?name=${encodeURIComponent(req.body.nombre)}&background=random&color=fff&size=200&bold=true`; }
        await db.run('INSERT INTO productos (nombre, precio, stock, imagen, categoria, club_id, deporte_id) VALUES (?, ?, ?, ?, ?, ?, ?)', [req.body.nombre, req.body.precio, req.body.stock, imagenFinal, req.body.categoria, req.body.club_id, req.body.deporte_id]); res.json({ success: true }); 
...
app.put('/productos/:id', verificarToken, upload.single('imagen_file'), async (req, res) => { 
    try { 
        ...
        if (imagenFinal) { await db.run('UPDATE productos SET nombre = ?, precio = ?, stock = ?, imagen = ?, categoria = ? WHERE id = ?', [req.body.nombre, req.body.precio, req.body.stock, imagenFinal, req.body.categoria, req.params.id]); } 
        else { await db.run('UPDATE productos SET nombre = ?, precio = ?, stock = ?, categoria = ? WHERE id = ?', [req.body.nombre, req.body.precio, req.body.stock, req.body.categoria, req.params.id]); }
        res.json({ success: true }); 
    } catch (e) { res.json({ success: false }); } 
});
...
app.post('/confirmar-venta', verificarToken, async (req, res) => {
    ...
    try {
        ...
        for (const item of items) { await db.run('UPDATE productos SET stock = stock - ? WHERE id = ?', [item.cantidad, item.id]); await db.run('INSERT INTO ventas_detalles (venta_id, producto_nombre, cantidad) VALUES (?, ?, ?)', [ventaId, item.nombre, item.cantidad]); }
...
app.get('/despacho/buscar/:codigo/:deporteId', verificarToken, async (req, res) => { try { const venta = await db.get(`SELECT id, codigo_retiro, estado_entrega, fecha FROM ventas WHERE codigo_retiro = ? AND deporte_id = ?`, [req.params.codigo.toUpperCase(), req.params.deporteId]); if (!venta) return res.json({ success: false, mensaje: "TICKET NO ENCONTRADO" }); if (venta.estado_entrega === 'ENTREGADO') return res.json({ success: false, mensaje: "❌ ESTE TICKET YA FUE ENTREGADO" }); venta.items = await db.all(`SELECT producto_nombre, cantidad FROM ventas_detalles WHERE venta_id = ?`, [venta.id]); res.json({ success: true, venta }); } catch (e) { res.status(500).json({error: e.message}); } });
app.put('/despacho/entregar/:id', verificarToken, async (req, res) => { try { await db.run(`UPDATE ventas SET estado_entrega = 'ENTREGADO' WHERE id = ?`, [req.params.id]); res.json({success:true}); } catch (e) { res.status(500).json({error: e.message}); } });

app.post('/gastos', verificarToken, async (req, res) => { try { await db.run('INSERT INTO gastos (descripcion, monto, caja_id, club_id, deporte_id, tipo) VALUES (?, ?, ?, ?, ?, ?)', [req.body.descripcion, req.body.monto, req.body.caja_id, req.body.club_id, req.body.deporte_id, req.body.tipo || 'GASTO']); res.json({ success: true }); } catch (e) { res.json({ success: false }); } });
